...
if %ERRORLEVEL% EQU 0 (
    ...
    wt -w "AutoAffiliate" nt --title "Appium Server" -d "%~dp0server" cmd /k "appium -p 4723" ; ^
       ...
       nt --title "Celery Default Core" -d "%~dp0server" cmd /k "timeout /t 5 >nul && .venv\Scripts\activate && celery -A app.core.celery_app worker --loglevel=info -Q default -E" ; ^
       nt --title "Celery Appium Node" -d "%~dp0server" cmd /k "timeout /t 5 >nul && .venv\Scripts\activate && celery -A app.core.celery_app worker -Q appium_phone --concurrency=1 -E" ; ^
